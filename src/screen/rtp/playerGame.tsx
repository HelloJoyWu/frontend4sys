...
const PlayerGameScreen = ({
  ...
  const [pickedGame, setPickedGame] = useState<any>();
  ...
  const [clearTimeOutState, setClearTimeOutState] = useState<any>();
  const [clearTimeOutCancelBtn, setClearTimeOutCancelBtn] = useState<any>();
  const [clearOverTime, setClearOverTime] = useState<any>();
  ...
  const setAmount = async () => {
    ...
    if (!pickedGame) {
      Alert.alert('Please select GameCode!');
      return;
    }
    ...
    if (accountText.length <= 0) {
      Alert.alert('Please enter player account!');
      return;
    }
  ...
  const fetchDayLatest = async (params: any, date: any) => {
    await authAxios
      .get('api/v1/inquiry/player/day/game/summary', {
        params: {
          game_type: pickedGame.gameType,
          game_code: pickedGame.value,
          agent_type: params.agent_type,
          agent_id: params.agent_id,
          player_account: accountText,
          from_time: date.from,
          to_time: date.to,
        },
      })
      ...
      <KeyboardAwareScrollView contentContainerStyle={{flexGrow: 1}}>
        <View style={styles.bodyBox}>
          <View style={{zIndex: 1, width: '85%'}}>
            ...
            {SetGameCodeDropdown(pickedGame, setPickedGame)}
            {SetAccountTextInputBox(accountText, setAccountText)}
            <TouchableOpacity
              onPress={() => setAmount()}
              style={{width: '100%'}}
              disabled={sendBtnDis}>
